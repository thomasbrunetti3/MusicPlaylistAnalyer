...
namespace MusicPlaylistAnalyzer
{
    ...
    {
        public static string GenerateText(List<MusicStatsList> musicStatsList)
        {
            ...
            if (moreThan200Plays.Count() > 0)
            {
                ...
                foreach (MusicStatsList musicStats in moreThan200Plays)
                {
            ...
            if (alsongs.Count() > 0)
            {
                foreach (MusicStatsList musicStats in alsongs)
                {
                    report += musicStats.ToString() + "\n\n";
                }
            }
            ...
            if (oldsongs.Count() > 0)
            {
                foreach (MusicStatsList musicStats in oldsongs)
                {
                    report += musicStats.ToString() + "\n\n";
                }
            }
            ...
            var longest = from musicStats in musicStatsList
                          where musicStats.Time ==
((from stats in musicStatsList select stats.Time).Max())
                          select musicStats;
